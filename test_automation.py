    ...
    while time.time() < deadline:
        ...
        try:
            ...
            if input_by_ph.count() > 0 and output_by_ph.count() > 0 and input_by_ph.is_visible() and output_by_ph.is_visible():
                action_candidates = page.locator("button").filter(has_text=re.compile(r"Transliterate", re.IGNORECASE))
                if action_candidates.count() > 0:
                    return input_by_ph, output_by_ph, action_candidates.last
        except Exception:
            pass
        ...
        try:
            ...
            if len(visible) >= 2:
                action_candidates = page.locator("button").filter(has_text=re.compile(r"Transliterate", re.IGNORECASE))
                if action_candidates.count() > 0:
                    return visible[0], visible[1], action_candidates.last
...
def run_test():
    ...
    try:
        wb = openpyxl.load_workbook(args.excel, keep_vba=False, rich_text=False)
    except Exception as e:
